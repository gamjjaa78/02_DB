...
--메인쿼리가 SELECT 문일때
--SELECT, FROM, WHERE, HAVONG 절에서 사용가능
...
SELECT SALARY FROM EMPLOYEE ORDER BY SALARY; --1,380,000
...
--부서별(부서없는 이 포함) 급여의 합계 중 가장 큰 부서의 부서명, 급여 합계를 조회
...
CASE WHEN EMP_ID IN (SELECT DISTINCT MANAGER_ID
					...
					THEN '사수'
					ELSE '사원'
					END 구분 --CASE~구분까지도 컬럼이기때문에 위 JOB_NAME옆에 반점 있어야돼
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE =DEPT_ID);
...
--1)직급이 과장인 직원들의 급여조회(서브쿼리)
SELECT SALARY
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME='과장';

--2)직급이 대리인 직원들의 사번,이름,직급,급여조회(메인쿼리)
...
--서브쿼리 중첩사용(응용!)
--LOCAION 테이블에서 NATIONAL_CODE가 KO인 경우의 LOCAL_CODE와
--DEPARTMENT 테이블의 LOCATION_ID와 동일한 DEPT_ID가
--EMPLOYEE 테이블의 DEPT_CODE와 동일한 사원을 구하시오,
--사원의 이름, 부서코드 조회

--1)LOCAION 테이블에서 NATIONAL_CODE가 KO인 경우의 LOCAL_CODE와
...
--LOCATION_ID가 가지고 있는 DEPT_ID 조회
SELECT DEPT_ID
FROM DEPARTMENT
...
---------------------연습문제---------------------
--1. 노옹철 사원과 같은 부서, 같은 직급인 사원을 조회(단,노옹철제회)
--사번, 이름, 부서코드, 직급코드, 부서명, 직급명 조회

SELECT * FROM EMPLOYEE;  --JOB CODE
SELECT * FROM JOB; --
...
--4. 다중행 다중열 서브쿼리
--서브쿼리 조회결과 행수와 열수가 여러개일때

--본인이 소속된 직급의 평균급여를 받고있는 직원의
--사번, 이름, 직급코드, 급여조회
...
--*상관 서브쿼리는 행마다 비교/검증필요할때 사용

--직급별 급여 평균보다 급여를 받이 받는 직원의
--이름, 직급코드, 급여 조회
...
WHERE SALARY > (SELECT AVG(SALARY)
		    	...
		    	--WHERE JOB_CODE='J2'의 J2가 변경될 수 있기 때문에 고정값안씀
...
--메인쿼리 (--사번, 이름, 부서코드, 부서명(NULL이면 '소속없음'), 직급명, 입사일 조회)
...
--6.스칼라 서브쿼리
--SELECT 절에 사용되는 서브쿼리, 결과를 1행만 반환
--SQL에서 단일 값을 '스칼라'라고함
...
-->SELECT 문의 해석순서때문에 급여 상위 5명이 아니라
--조회순서 상위 5명끼리의 급여 순위 정렬돼 조회
...
--2)메인쿼리 조회시 인라인 뷰를 이용해 급여상위 5명까지 조회
SELECT ROWNUM, EMP_NAME, SALARY
FROM (SELECT EMP_NAME, SALARY
	FROM EMPLOYEE
	ORDER BY SALARY DESC) --해석순위1위 FROM절에서 전체직원의 SALARY 내림차순
...
--RANK() OVER:동일한 순위 이후의 동수를 동일한 인원수만큼 건너뛰고 순위계산
-->공동1위가 2명이면 다음 순위는 2위가 아니라 3위로 조회됨
...
SELECT DEPT_CODE, JOB_CODE --D9,J2
FROM EMPLOYEE
WHERE EMP_NAME='노옹철';
...
-- 사번, 이름, 부서코드, 직급코드, 고용일
SELECT DEPT_CODE, JOB_CODE --D6,J3
FROM EMPLOYEE
WHERE EXTRACT(YEAR FROM HIRE_DATE) = 2000;
