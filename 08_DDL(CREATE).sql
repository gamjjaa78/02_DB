...
-- 테이블로 생성된 객체는 DROP 구문을 통해 제거 할 수 있음 (유저 등도 가능)
-- DROP TABLE MEMBER;
-- DROP USER MEMBER;
...
--테이블 생성시 해당 컬럼에 지정된 DEFAULT 값으로 삽입된다!*

COMMIT;

--.가입일->INSERT 시 미작성하는 경우 DEFAULT 값이 반영되는지 확인
INSERT INTO MEMBER
VALUES('MEM02', 'QWER1234', '김영희', '000123-2234567', SYSDATE);
...
INSERT INTO MEMBER2 VALUES('MEM01', 'PASS_01', '고길동', 010111222);

SELECT * FROM MEMBER2;

-->NUMBER 타입 컬럼에 데이터 삽입시
--제일 앞에 0이 있으면 이를 자동으로 제거함
-->전화번호, 주민번호처럼 숫자로만 돼있는 데이터라도
--0으로 시작할 가능성이 있다면 CHAR, VARCHAR2같은 문자형 이용해야함!!

------------------------------------

--제약조건(CONSTRAINTS)
--사용자가 원하는 조건의 데이터만 유지하기 위해서 특정컬럼에 설정하는 제약
-->중복데이터 X

--입력데이터에 문제가 없는지 자동을 검사하는 목적
--데이터의 수정/삭제 가능여부 검사하는 목적으로 함
-->제약조건을 위배하는 DML 구문은 수행할 수 없다

--제약조건 종류
--PRIMARY KEY, NOT NULL, UNIQUE, CHECH, FOREIGN KEY

--1. NOT NULL
--해당 컬럼에 반드시 값이 기록되어야 하는 경우 사용
--삽입/수정시 NULL값을 허용하지 않도록 컬럼레벨에서 제한

--*컬럼레벨:테이블 생성시 컬럼을 정의하는 부분에 작성하는 것

CREATE TABLE USER_USED_NN (
	USER_NO NUMBER NOT NULL, --모든 사용자는 사용자번호가 있어야한다
	USER_ID VARCHAR2(20),
	USER_PWD VARCHAR2(20),
	USER_NAME VARCHAR2(30),
	GENDER VARCHAR2(10),
	PHONE VARCHAR2(20),
	EMAIL VARCHAR2(20) --마지막 컬럼 작성부분까지 컬럼 레벨
	--테이블 레벨
);
...
(1, 'USER01', 'PASS01', '홍길동', '남', '010-1111-2222', 'HONG@OR.KR');

INSERT INTO USER_USED_NN VALUES
(NULL, 'USER01', 'PASS01', '홍길동', '남', '010-1111-2222', 'HONG@OR.KR');
--ORA-01400: NULL을 ("KH_CHJ"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다

-->NOT NULL 제약조건 위배돼 오류발생!

----------------------------------------

--2.UNIQUE 제약조건
--컬럼에 입력값에 대해 중복제한하는 제약조건
--컬럼레벨에서 설정가능, 테이블레벨에서 설정가능
--단 UNIQUE 제약조건이 설정된 컬럼에 NULL값은 중복삽입가능

--*테이블레벨:테이블생성시 컬럼정의가 끝난 후 마지막에 작성
...
CREATE TABLE USER_USED_PK (
	...
	EMAIL VARCHAR2(50)
	--테이블 레벨
	--, CONSTRAINT USER_NO_PK PRIMARY KEY(USER_NO)  --제약조건, 제약조건 이름, 제약조건 종류, 제약걸 컬럼이름
	);
...
INSERT INTO USER_USED_PK VALUES
(1, 'USER02', 'PASS02', '이순신', '남', '010-1111-3333', 'LEE@OR.KR');
--ORA-00001: 무결성 제약 조건(KH_CHJ.USER_NO_PK)에 위배됩니다
-->기본키 중복 오류!

INSERT INTO USER_USED_PK VALUES
(NULL, 'USER02', 'PASS02', '이순신', '남', '010-1111-3333', 'LEE@OR.KR');
--ORA-01400: NULL을 ("KH_CHJ"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
-->기본키 NULL 이므로 오류!

INSERT INTO USER_USED_PK VALUES
(2, 'USER02', 'PASS02', '이순신', '남', '010-1111-3333', 'LEE@OR.KR');
--무결한 데이터, 삽입가능!
...
(1, 'USER01', 'PASS01', '홍길동', '남', '010-1111-3333', 'HONG@OR.KR');

INSERT INTO USER_USED_PK2 VALUES
(1, 'USER02', 'PASS01', '홍길동', '남', '010-1111-3333', 'HONG@OR.KR');

INSERT INTO USER_USED_PK2 VALUES
(2, 'USER02', 'PASS01', '홍길동', '남', '010-1111-3333', 'HONG@OR.KR');

INSERT INTO USER_USED_PK2 VALUES
(2, 'USER02', 'PASS01', '홍길동', '남', '010-1111-3333', 'HONG@OR.KR');
--ORA-00001: 무결성 제약 조건(KH_CHJ.PK_USERNO_USERID)에 위배됩니다
-->UWER_NO, USER_ID 둘 다 중복됐을때만 제약조건 위배!
...
INSERT INTO USER_USED_PK2 VALUES
(3, NULL, 'PASS01', '홍길동', '남', '010-1111-3333', 'HONG@OR.KR');
--ORA-01400: NULL을 ("KH_CHJ"."USER_USED_PK2"."USER_ID") 안에 삽입할 수 없습니다
-->둘 중 하나라도 NULL 이면 위배!!!
...
(1, 'USER01', 'PASS01', '홍길동', 
'남자', '010-1234-5678', 'hong@kr.or.kr');
--ORA-02290: 체크 제약조건(KH_CHJ.GENDER_CHECK)이 위배되었습니다
...
INSERT INTO USER_USED_CHECK VALUES
(2, 'USER02', 'PASS02', '유관순', 
'여', '010-1234-5670', 'YOO@kr.or.kr');


-->GENDER 제약조건으로 CHECK 제약조건으로 '남' 또는 '여'만 삽입가능하도록 설정
-->이 외 값이 들어오면 체크 제약조건 위배돼 에러발생!!!!