...
--VIEW 생성위해 권한필요!!
--(SYS 계정 접속)
--VIEW 생성권한을 부여 (SYS->kh_XXX 계정으로)
GRANT CREATE VIEW TO kh_chj;
...
NOMINVALUE --최소값없음
NOCYCLE --반복안함
NOCACHE; --미리 만들어둘 시퀀스번호 없음

--시퀀스 테스트할 테이블 생성
CREATE TABLE TB_TEST(
TEST_N0 NUMBER PRIMARY KEY,
...
-->가장 최근 호출된 NEXTVAL 값을 반환함을 뜻함
-->NESTVAL 를 호출한 적이 없어 오류 발생!!
...
ALTER SEQUENCE SEQ_TEST_NO RESTART START WITH 100; --시퀀스 재실행, 롤백은안됨
...
UPDATE TB_TEST SET
TEST_N0 = SEQ_TEST_NO.NEXTVAL
WHERE TEST_NAME='짱구';
-->짱구의 TEST_N0값을 150까지 증가시키고 나서 또한번 호출
...
--ROWID:오라클에서 각 행의 고유한 주소를 나타내는 가상컬럼
/*ROWID와 INDEX의 관계?
 * 인덱스가 ROWID를 저장함
